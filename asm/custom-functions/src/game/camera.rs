use crate::system::{
    button::{
        ...
        set_buttons_pressed, set_stick_pos, A, B, Buttons, C, DPAD_LEFT, DPAD_RIGHT, Z,
    },
...
fn consume_gameplay_input() {
    ...
    pressed.remove(A | B | C | Z | DPAD_LEFT | DPAD_RIGHT);
    set_buttons_pressed(pressed);
    let mut down = buttons_down();
    down.remove(A | B | C | Z | DPAD_LEFT | DPAD_RIGHT);
...
pub fn update(free_cam_active: bool, freeze_camera_active: bool) {
    ...
    let apply_override = unsafe { free_cam_active || freeze_camera_active || EXTERNAL_CAMERA_OVERRIDE_ACTIVE };
    if apply_override {
